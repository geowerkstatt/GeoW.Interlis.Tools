...
public class InterlisReaderExpressionTest
{
    ...
    public void ReadClassConstExpression()
    {
        AssertReadRule(">Model.Topic.Class",
            new PathExpression
            {
                Path =
                {
                    new ReferencePathElement { Value = new Reference<IInterlisDefinition> { Path = { "Model", "Topic", "Class" }, ReferenceLocation = new RangePosition(0, 1, 0, 18) } },
                }
            });
    }
    ...
    public void ReadAttributePathConstExpression()
    {
        AssertReadRule(">>Model.Topic.Class->Attribute",
            new PathExpression
            {
                Path =
                {
                    new ReferencePathElement { Value = new Reference<IInterlisDefinition> { Path = { "Model", "Topic", "Class" }, ReferenceLocation = new RangePosition(0, 2, 0, 19) } },
    ...
    public void ReadFunctionCallExpression()
    {
        AssertReadRule("len(textAttr)",
            new FunctionCall
            {
                FunctionDef = new Reference<FunctionDef> { Path = { "len" }, ReferenceLocation = new RangePosition(0, 0, 0, 3) },
